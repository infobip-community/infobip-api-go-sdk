...
func TestGetDomainValidReq(t *testing.T) {
	apiKey := "apiKey"
	rawJSONResp := []byte(`
		{
		  "domainId": 1,
	...
	var expectedResp models.GetEmailDomainResponse
	err := json.Unmarshal(rawJSONResp, &expectedResp)
	require.NoError(t, err)

	serv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		require.Equal(t, http.MethodGet, r.Method)
		assert.True(t, strings.Contains(r.URL.Path, getDomainPath))
	...
	resp, respDetails, err := email.GetDomain(context.Background(), "test-domain.com")
