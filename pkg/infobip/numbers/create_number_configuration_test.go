...
func TestCreateNumberConfigurationValidReq(t *testing.T) {
	rawJSONResp := []byte(`
	{
	...
	serv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		assert.True(t, strings.HasSuffix(r.URL.Path, fmt.Sprintf(createNumberConfigurationsPath, numberKey)))
	...
	msgResp, respDetails, err := number.CreateNumberConfiguration(context.Background(), numberKey,
		models.NumberConfiguration{
			Key: "key",
